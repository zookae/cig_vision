...
% % AZ: ``pervasive'' games? may be too much jargon, but seems to more succinctly convey the point
...
A primary concerns for these researchers has been (a) ways to appropriately represent game content to suit generation algorithms, while (b) providing means for users to interact with generation systems to author desired content and outcomes.
%
...
%\texttt{The key to lifelong agents in games may be algorithms that learn from interactions with individual players. \mytodo{player modeling?}
%Lifelong agents must be able to adapt to new content from patches, expansions, downloadable content, and other incremental updates to the game the agent inhabits.
%Research in machine learning for {\em lifelong learning} investigates the transfer of knowledge between particular tasks, continually learning and refining knowledge, uncovering representations for complex information, and incorporating guidance or feedback from humans. Addressing these challenges in the continually evolving domain of games promises benefits for developers running live games, an audience not yet unaddressed by game AI research.

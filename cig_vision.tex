...
\begin{abstract}
%\boldmath
Persistent online games, digital distribution platforms, social and mobile games, and the emergence of new business models have pushed game development to put heavier emphasis on the live operation of a game.
...
We characterize a confluence of four major forces at play in the games industry today, together producing a wealth of data that opens unique research opportunities and challenges for game AI producers.
...
Players are more diverse, have access to games in more places and at more times, and produce more data and content for developers than ever before. 
...
\item Developers and players are increasingly interested in coupling the real world and virtual world %; and
...
The consequence of these forces is a profound opportunity for Artificial Intelligence, Computational Intelligence, and Machine Learning to play an even greater role in the development of computer games and the delivery of engaging real-time experiences to players. 
...
% % AZ: this paragraph seems a bit disconnected from the remaining discussion...
% % could reframe a bit in terms of what ``human-level'' means to each paradigm
% % 1.0: playing game
% % 2.0: designing game world
% % 3.0: design universe of games / meta-verse of games
...
%A second notion of ``human-level'' intelligence in games is as a designer and developer. 
...
\mytodo{SC competition---more complicated, learning/smart companions---human-centered / self-correcting / more robust, GGP---more general}
Recent advances in Game AI 1.0 have seen agents developed to for more complex domains, exemplified by the Starcraft AI competition [REF]. 
More general game AI systems have been pursued through the General Game Playing competition where agents must reason on a rule-set defining an \textit{a priori} unknown new game within a massive space of possible games \cite{genesereth2005:general-game-playing}. 
Game AI 1.0 is also addressing robustness through development of learning companion agents able to intelligently coordinate with players [REF: AIIDE12 POMDP paper?].
...
Historically, content generation in the form of level generation in games such as Rogue has been around nearly as long as 1.0 Game AI techniques. 
\mytodo{more? AZ: should probably give a clear example of something that adds to existing 1.0 methods, rather than noting the two exist in parallel. below is my attempt but it isn't academic citation (I can't think of any ATM that do the same thing, but I imagine they exist)}
Recent developments have seen 1.0 agents being developed that can cope with the procedural worlds produced by 2.0 systems, such as the \mytodo{XXX} game [REF: GDC AI postmortem on agents in procedural spaces]. 

{\em Game adaptation} combines content generation and player modeling to enable AI designers to tailor games to individual players. Game generation takes this adaptation process to its extreme by creating entire games driven by (real or simulated) feedback. Game adaptation emphasizes a closed loop of modeling player actions and adjust game content based on design goals for player behavior \cite{smith2012:refraction}, player skill acquisition \cite{andersen2013:trace}, or maximizing player enjoyment \cite{yu2012:prefix-based, thue2007:storytell-pm, shaker2010:platformer-gen}.
%, or preferences [Yu]. 
...
Game data visualization  focuses on designer understanding game systems to inform the design of game systems and AI agents \cite{wallner2013:gameplay-viz-analysis}. Player modeling research examines methods to describe and predict player behavior, potentially to be used by designers or automated AI systems in response \cite{smith2011:playermodel}. While not a traditional domain of game AI, player modeling has becoming increasingly prominent as AI agents shift to learning to adapt to players. Machine learning (e.g. \cite{harrison2011:wow-seq-pred, weber2011:playermodel}) and evolutionary computing (see \cite{yannakakis2011:edpcg}) are the primary areas currently being employed to perform these modeling tasks.
...
In our metaphor, producers concern themselves with the entire set of games and game content being made by a company, including managing player communities. 
...
AI designers emphasized player models that captured the activity of a single user as a single character;
AI producers distinguish characters (in-game characters or personas) from players (agents manipulating those characters). 
...
Games are no longer sharply bound by a single delivered product and game AI should respond by incorporating these newly opened borders for new research domains.
Effectively responding to these challenges will require new methods to leverage the masses of data being produced by games to improve interactive experiences.
...
In each case these forces provide unique opportunities for data-driven approaches to game AI that enhance player experience through richer sources of information and emerging modes of game-based interaction. 
Rather than replace earlier paradigms of game AI, these four forces introduce novel opportunities to address concerns and enhance techniques from earlier paradigms.
%
...
Many contemporary games have high ceilings for player skills to build up to---through complex underlying systems or ever-evolving multiplayer competitions---but lose players early in the game due to the disparate skills players bring to games.
...
Existing research in Intelligent Tutoring Systems \mytodo{ITS overview paper} presents one vision for gameplay support \cite{vanlehn2006:behav-its}. 
...
Adapting these techniques for gameplay support requires capturing the long-term effects of interventions on player engagement, richer models of player skills related to various gameplay domains, and techniques to intelligently reuse high quality human-authored content where possible.
...
Connecting characters and gameplay from a single player across multiple games opens new opportunities in (1) cross-game agents, (2) cross-game content generation, and (3) automated online game designs experiments. 
...
Taken to its extreme, such design knowledge can lead to generating new games that span genres, moving beyond the genre-focused efforts currently being explored.
...
\subsubsectionx{How can a Game AI system enhance user-generated content?}
%
...
%How can PCG systems incentivize human contributions when content has nuanced semantic or aesthetic dimensions?
...
\mytodo{cross-game curator? automated exhibition generation?}
...
Beyond this information, games must also account for other aspects of player context including social circumstances or economic situation . Together this additional contextual information and output opportunities open research avenues related to (1) incorporating real-world context into games, (2) using out-of-game social network data in games and (3) using games to pro-actively sense real-world information.
...
%
Modeling in-game social interactions, social networks, and player communities can enable game AI agents to interact with players as part of a social world that (partially) overlaps the in-game world. Current research in online game social networks has explored detecting social information including group identities \cite{grappiolo2013:group-identification}, shared housing networks \cite{ahmad2011:goldmining-hypergraph}, or real-money trade \cite{fujita2011:rmt-detect}. 
...
Overall, methods to relate in-game interactions to out-of-game social interaction remain unexplored---much research on genres beyond MMOGs and interactions beyond documented fraud or friendship remain. 
...
% problem analogy issue: can system learn generic mapping from in-game task to out-game info needed? cross-game employ?
